...
// ErrSetupTokenInvalid covers a wrong, expired or already-superseded token, with one reason
// given to the caller regardless of which: there is no oracle worth closing here the way there
// is for invites, but also no reason to distinguish the cases.
...
// Pending reports whether the panel still needs its first admin, from COUNT(users) rather
// than a kv flag that could drift from it. Callers needing this on every request should cache
// it and invalidate on Setup succeeding, not call this per request (11 §5.3).
...
// PrintToken generates a fresh token and prints it framed to w, if the panel is still pending.
// Called once per process start (10 §6), never on a timer, so a token is refreshed only by
...
// Setup verifies token and creates the first admin. As with a login, the token never appears
// in a log line here (D10).
//
// The pending check here is a courtesy, a fast rejection before the argon2id hash runs. The
// guarantee that only one admin is ever created is store.CreateFirstAdmin's, which re-checks
// atomically on the writer's single connection.
